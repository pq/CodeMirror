...
CodeMirror.defineMode("dart", function(config, parserConfig) {
  ...
  var keywords = function(){
    ...
    var operator = kw("operator"), atom = {type: "atom", style: "atom"}, directive = {type: "directive", style: "keyword"};
    return {
      //keywords
      "assert" : kw("assert"),
      "break": C,
  ...
  function dartTokenStartRawString(stream, state) {
    ...
    return chain(stream, state, dartTokenBase);
  }
  ...
  function vardef1(type, value) {
    if (type == "variable"){return cont(vardef2);}
    return cont();
  }
